...
Route::prefix(getTenantPrefix())
    ->middleware(['is_valid_tenant'])
    ->group(function () use ($frontend) {
        Route::controller(SubscribeController::class)->group(function () {
            Route::get('/subscribe', 'index')->name('subscribePage');
            Route::get('/subscribe/checkout', 'checkout')->name('subscribe.checkout');
            Route::post('/subscribe/process-payment', 'processPayment')->name('subscribe.processPayment');
        });

        Route::middleware(['check_subscription'])->group(function () use ($frontend) {
            ...
            Route::get('/blog/{slug}', function (string $tenant, string $slug) use ($frontend) {
            ...
            Route::get('/blog-author/{slug}', function (string $tenant, string $slug) use ($frontend) {
            ...
            Route::get('/store/{slug}', function (string $tenant, string $slug) use ($frontend) {
            ...
            Route::get('/category/{slug}', function (string $tenant, string $slug) use ($frontend) {
